...
4. **Try `brain_doctor`** — `python3 ~/.claude/scripts/brain_doctor.py` runs a battery of read-only health checks (plus one per synced arm) if something feels off after install or sync.
