...
> **Your AI agent forgets who you are and mixes your worlds. Octorato is the open-source brain layer that fixes that: one file-native "brain" (<!--canon:skills.count-->230+<!--/canon--> skills and <!--canon:agents.count-->160+<!--/canon--> specialist agents in plain markdown under git) that one operator runs across many isolated "arms": a client, a project, a topic, a course, anything you keep separate.**
...
- [x] **Per-process quotas** (v8 kernel: `registry/kernel.yaml` declares the slot, gitignored `company/config/kernel.json` carries the caps, `scripts/g__pretool__kernel.py` refuses the call that would exceed them, so spend is capped per arm AND per process, subagents included)
