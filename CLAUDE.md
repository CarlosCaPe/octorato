...
- **Receipt ledger** (`scripts/receipt_ledger.py`, gitignored `~/.claude/.cache/receipts/`): `scripts/r__posttool__receipt-seek.py` records every chat, mail or memory lookup with its harness-supplied `tool_use_id`; a consumer accepts the receipt only if that id is a real tool_use in the current turn of the transcript, so a hand-typed line names nothing. `brain_doctor` records a gate receipt (git HEAD plus the git tree hash of `scripts/`, `registry/` and `hooks.json`) when gate-liveness passes on a clean tree; `.githooks/pre-push` runs `brain_doctor --gate-receipt` so a push and a receipt are one event, and an uncommitted edit under the gate surfaces voids the receipt.
