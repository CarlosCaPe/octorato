...
This is why every answer ends with a real **Provenance footer** (Basis · Engine · Touched · Verified): provenance over performance. Wired, not remembered: `scripts/g__stop__provenance-engine.py` (Registry `COMMS.provenance-footer`, selftest-proven) blocks once when a substantive reply leaves without a footer that names its Engine; `provenance-ok` on a line is the deliberate waiver. It ships UNARMED, per the registry convention for new gates: arming it hot would block every live session at once, so the operator registers it in `hooks.json` under Stop (next to `g__stop__draft-promise.py`) and flips its `firing_event` to `Stop`. Naming the engine is the property that separates a connector from a bot whose model nobody can name. Instance-specific identity — names, banners, market positioning, "superiority" copy — lives in that instance's own brain, **never in this generic one**.
