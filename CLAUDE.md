...
- **QA receipt** (`scripts/r__subagent-stop__qa-receipt.py`, SubagentStop): a QA subagent that ends with `QA-VERDICT: PASS|FAIL|NEEDS-WORK` and `QA-SCOPE: PR #<n>` gets its verdict recorded with the harness-written agent transcript path; `qa-merge-gate` requires, on top of the operator's `OCTO_MERGE_APPROVE`, a PASS receipt for that PR whose transcript lives under the harness projects dir, whose LAST assistant text re-parses to PASS with the PR named as a whole token, and whose agent is a QA persona. "QA approved" typed by the main loop is not a receipt. `OCTO_QA_OK=1` is an explicit bypass of the QA receipt for the PR named in `OCTO_MERGE_APPROVE`, never of the approval itself: with a different PR number, or with no `OCTO_MERGE_APPROVE` at all, the merge is still denied (it used to be a blanket, and that let a merge of the wrong PR through). Its scope is per COMMAND, not "one-time": nothing consumes it, so it keeps waiving every merge of that same PR while the shell keeps it exported, and unsetting it is the operator's job.
