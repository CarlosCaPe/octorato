...
# ── quiescence guard (co-tenancy's blind spot: writers INSIDE our own session) ──
#
# WHY THIS EXISTS, and why the co-tenancy guard above does not cover it.
# _dimensions_seen() counts SESSIONS by heartbeat, and a subagent inherits its
# parent's session id. So does the lane registry in dimension-awareness-hook.py:
# lanes are claimed per session, so two writers of the SAME session never collide.
# Net effect before this guard: the brain was armored against another terminal of
# the operator's and BLIND to its own in-flight workers. Real incident 2026-08-11 —
# a subagent was mid-write on an 864-line, unreviewed security gate when the main
# agent ran `ai-sync "reflexion diaria"`; because BRAIN_PATHS stages whole
# DIRECTORIES (`git add scripts/`), the half-finished gate rode into a 55-file,
# 2772-insertion commit under an unrelated message, and went public.
#
# WHAT THIS IS: a quiescence check. Before staging, stat every file that would
# enter the index; if any was written inside a recent window, a writer is probably
# still alive, so abort and NAME the hot files.
#
# HONEST LIMIT — THIS IS A HEURISTIC, NOT A PROOF OF LIVENESS.
# A writer that pauses longer than the window slips through: an agent that thinks
# for 3 minutes between two Edits looks identical to a finished one. Measured on
# this brain's own subagent transcripts (166 consecutive Write/Edit gaps):
# p50 12.5s, p75 30s, p90 128s, p95 244s, max 1250s. So a 120s window catches
# roughly 9 of 10 gaps of a continuously-working agent and misses the long-think
# tail by construction. It is a smoke alarm, not a lock.
#
# DETERMINISTIC ALTERNATIVES CONSIDERED AND REJECTED (2026-08-11 census):
#   * ~/.claude/projects/<slug>/<session>/subagents/agent-*.jsonl — Claude Code
#     DOES keep one transcript per subagent, and a finished one ends on an
#     assistant record with stop_reason "end_turn" while a live one ends mid
#     tool-loop. That is a genuine liveness signal, and it was rejected anyway:
#     (a) it is an undocumented internal layout (observed on 2.1.225) that a
#     harness upgrade can move, and a guard whose primary signal can silently
#     vanish is fail-OPEN, which is the wrong failure direction here; (b) it says
#     a subagent is alive but not WHICH FILES it will write, so a read-only
#     research subagent would block every push — and since this brain routes most
#     work through subagents by policy, that guard would be disabled within a day;
#     (c) it needs the harness project-slug, which this script cannot derive.
#   * agent-*.meta.json — written once at spawn; carries agentType/description/
#     toolUseId/spawnDepth and NO status field, so it never reports completion.
#   * lanes in connectome/sessions.json — per SESSION, so blind to a subagent by
#     the same defect this guard exists to cover.
#   * open file descriptors (/proc, lsof) — Write/Edit are atomic short writes,
#     the fd is closed long before the push runs.
# mtime is the only signal that is per-FILE, harness-independent, and cannot go
# silently absent. Hence: heuristic primary, documented blind spot.
_QUIESCE_DEFAULT = 120  # seconds; see the measured gap distribution above

# Files THIS SCRIPT rewrites on every run. Counting them would abort every push,
# and a guard that always fires gets switched off — so they are excluded by
# provenance (machine-regenerated, no human or agent author), never by type.
#   docs/CAPABILITIES.md — regenerated by scripts/capability_manifest.py a few
#   lines below, deliberately BEFORE the add loop so the fresh manifest lands in
#   this commit. It is the only file that script writes (verified: single
#   write_text in capability_manifest.py) and it is therefore hot by construction.
# Not listed, and deliberately so:
#   connectome/sessions.json — rewritten by the dimension hook on every prompt,
#   so it would be a permanent false positive, but it is gitignored
#   (.gitignore:237) and never reaches the index. No exclusion needed.
#   neural_map.json — gitignored too, and regenerated AFTER the commit.
_QUIESCE_SELF_WRITTEN = frozenset({"docs/CAPABILITIES.md"})

# cycle() checks BEFORE its pull and sets this so push() does not re-check after.
# `git pull --rebase --autostash` stashes and restores the dirty tree, and a
# `git stash pop` REWRITES mtimes to now (verified empirically): every dirty file
# reads as 0s old afterwards. Re-running the check post-pull would abort 100% of
# ai-sync runs on a dirty tree — the exact "always fires, gets disabled" failure.
_QUIESCE_ALREADY_CHECKED = False


def _quiesce_window() -> int:
    """Quiescence window in seconds. Override with OCTO_QUIESCE_WINDOW."""
    try:
        return max(int(os.environ.get("OCTO_QUIESCE_WINDOW", _QUIESCE_DEFAULT)), 0)
    except (ValueError, TypeError):
        return _QUIESCE_DEFAULT


def _index_candidates() -> tuple[list[str], bool]:
    """Repo-relative paths that would enter the index, and an ok flag.

    Mirrors the staging below: everything git reports as changed or untracked
    that sits under a BRAIN_PATHS entry. Uses -z so paths with spaces, quotes or
    non-ASCII are never mangled by git's C-style quoting.

    Returns (paths, ok). ok=False means we could not enumerate — the caller must
    treat that as UNSAFE, never as "nothing to check".
    """
    code, out, _ = git("status", "--porcelain", "-z", "--untracked-files=all")
    if code != 0:
        return [], False
    ...
    while i < len(fields):
        ...
        # Rename/copy entries are followed by their ORIGIN path in the next
        # field. The origin is a deletion, not a live write, so skip it.
        if "R" in xy or "C" in xy:
            i += 1
        if not path:
            continue
        for base in BRAIN_PATHS:
            if path == base or (base.endswith("/") and path.startswith(base)):
                paths.append(path)
                break
    return paths, True


def _hot_files(window: int) -> tuple[list, bool]:
    """[(path, age_seconds), ...] written inside `window`, plus an ok flag.

    FAIL-CLOSED, unlike the co-tenancy guard next door: an unreadable mtime is
    reported as HOT (age -1), and an unusable `git status` returns ok=False.
    A guard that cannot see is not a guard that may wave things through — the
    thing it protects (someone else's unfinished work) is unrecoverable once
    pushed to a public repo.
    """
    candidates, ok = _index_candidates()
    if not ok:
        return [], False
    now = time.time()
    hot = []
    for rel in candidates:
        if rel in _QUIESCE_SELF_WRITTEN:
            continue
        try:
            age = now - (CLAUDE / rel).stat().st_mtime
        except OSError:
            hot.append((rel, -1))  # cannot stat → assume a writer holds it
            continue
        if age < window:
            hot.append((rel, int(max(age, 0))))
    hot.sort(key=lambda t: (t[1] if t[1] >= 0 else -1))
    return hot, True


def _quiescence_guard(verb: str) -> int:
    """0 = tree is quiet, proceed. 1 = a writer looks alive, abort.

    Override is deliberately the same shape as OCTO_ALLOW_SHARED: one env var,
    set consciously by whoever owns the tree, no flag on the command line that a
    wrapper could carry along by accident.
    """
    global _QUIESCE_ALREADY_CHECKED
    window = _quiesce_window()
    if window == 0 or os.environ.get("OCTO_ALLOW_HOT") == "1":
        _QUIESCE_ALREADY_CHECKED = True
        return 0
    hot, ok = _hot_files(window)
    _QUIESCE_ALREADY_CHECKED = True
    if ok and not hot:
        return 0
    if not ok:
        err(f"⚠ {verb} aborted: could not read the working tree to check for live writers.")
        err("   Fail-closed: refusing to stage a tree whose state is unknown.")
        err(f"   Override (only if you own the whole tree): OCTO_ALLOW_HOT=1 {verb}")
        return 1
    err(f"⚠ {verb} aborted: {len(hot)} file(s) written in the last {window}s — "
        f"a writer is probably still working:")
    for rel, age in hot[:15]:
        when = "mtime unreadable" if age < 0 else f"{age}s ago"
        err(f"     {rel}  ({when})")
    if len(hot) > 15:
        err(f"     … and {len(hot) - 15} more")
    err("   ai-push stages whole BRAIN_PATHS directories, so an unfinished file")
    err("   in one of them rides into this commit under an unrelated message.")
    err("   A subagent of THIS session is invisible to the co-tenancy guard above:")
    err("   it shares our session id, so it is neither a second dimension nor a")
    err("   lane conflict. This is the only check that sees it.")
    err("   Wait for the writer to finish, then retry — or, if you know nothing")
    err("   is writing (you are the only writer and these edits are yours):")
    err(f"       OCTO_ALLOW_HOT=1 {verb}")
    err(f"   Widen/narrow the window instead: OCTO_QUIESCE_WINDOW=<seconds> {verb}")
    return 1
...
def push(args) -> int:
    ...
    # Quiescence guard: the co-tenancy check above counts SESSIONS, and our own
    # subagents share our session id, so they are invisible to it. Skipped when
    # cycle() already ran it pre-pull — see _QUIESCE_ALREADY_CHECKED.
    if not _QUIESCE_ALREADY_CHECKED and _quiescence_guard("ai-push") != 0:
        return 1
...
def cycle(args) -> int:
    ...
    # Quiescence guard runs HERE, before the first pull, and never again.
    # `pull --rebase --autostash` restores the dirty tree via `git stash pop`,
    # which rewrites every dirty file's mtime to now — after it, mtime carries no
    # information at all. Checking pre-pull is the only placement where the
    # signal still exists. The incident this guard closes came in through exactly
    # this verb (`ai-sync "reflexion diaria"`), so cycle must carry the check.
    if _quiescence_guard("ai-sync") != 0:
        return 1
...
def _selftest_quiescence() -> int:
    """Liveness proof for the quiescence guard (registry ARCHITECTURE.session-isolation).

    Time-and-state dependent like the dimension hook's own selftest, so a static
    committed fixture cannot express it: we build a throwaway repo at runtime and
    drive the REAL _hot_files()/_quiescence_guard() against it.

    Three assertions, and the guard must get all three right:
      violation  a file freshly written inside a BRAIN_PATHS DIRECTORY  → abort, named
      benign     the same tree with every mtime aged past the window    → proceed
      benign     docs/CAPABILITIES.md fresh (this script regenerates it)→ proceed
    """
    import shutil
    import tempfile
    global CLAUDE
    real_claude = CLAUDE
    sandbox = Path(tempfile.mkdtemp(prefix="quiesce-selftest-"))
    ok = True
    try:
        CLAUDE = sandbox
        ...
        for a in ("init", "-q"), ("config", "user.email", "t@t"), ("config", "user.name", "t"):
            git(*a)
        (sandbox / "scripts" / "settled.py").write_text("# settled\n", encoding="utf-8")
        git("add", "-A")
        git("commit", "-qm", "base")

        old = time.time() - 86400

        # 1. VIOLATION — a subagent is mid-write inside scripts/ (a BRAIN_PATHS dir)
        (sandbox / "scripts" / "g__inflight__gate.py").write_text("# half\n", encoding="utf-8")
        hot, hok = _hot_files(_QUIESCE_DEFAULT)
        names = [p for p, _ in hot]
        if not hok or "scripts/g__inflight__gate.py" not in names:
            print(f"selftest FAIL: in-flight file not reported hot (got {names})",
                  file=sys.stderr)
            ok = False
        env_had = os.environ.pop("OCTO_ALLOW_HOT", None)
        if _quiescence_guard("selftest") == 0:
            print("selftest FAIL: hot tree did NOT abort", file=sys.stderr)
            ok = False

        # 2. BENIGN — same changes, all settled well past the window
        for p in (sandbox / "scripts").glob("*.py"):
            os.utime(p, (old, old))
        globals()["_QUIESCE_ALREADY_CHECKED"] = False
        if _quiescence_guard("selftest") != 0:
            print("selftest FAIL: settled tree WAS aborted", file=sys.stderr)
            ok = False

        # 3. BENIGN — the manifest this script itself regenerates is always fresh
        (sandbox / "docs" / "CAPABILITIES.md").write_text("# regenerated\n", encoding="utf-8")
        globals()["_QUIESCE_ALREADY_CHECKED"] = False
        if _quiescence_guard("selftest") != 0:
            print("selftest FAIL: fresh docs/CAPABILITIES.md tripped the guard",
                  file=sys.stderr)
            ok = False
        if env_had is not None:
            os.environ["OCTO_ALLOW_HOT"] = env_had
    finally:
        CLAUDE = real_claude
        globals()["_QUIESCE_ALREADY_CHECKED"] = False
        shutil.rmtree(sandbox, ignore_errors=True)
    if ok:
        print("selftest PASS: in-flight write aborts, settled tree proceeds, "
              "self-regenerated manifest exempt (ai_sync.py quiescence guard)")
    return 0 if ok else 1


def main() -> int:
    if "--selftest" in sys.argv:
        return _selftest_quiescence()
