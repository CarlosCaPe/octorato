...
# Resolve the brain from THIS script's location, not ~/.claude: run from a
# dimension worktree or a CI checkout, the home default made --check compare
# this checkout's scan against the OTHER checkout's doc and always report
# stale. Same fix brain-stats.py carries (issue #113). Env override stands.
CLAUDE_DIR = Path(os.environ.get("CLAUDE_DIR", Path(__file__).resolve().parent.parent))
...
def _today() -> str:
    """Today in UTC, ISO date. Isolated so --check can normalize it away:
    the stamp must not make an otherwise-identical document read as stale."""
...
def render(inv, all_tools_agents, unscoped_agents):
    ...
    out.append(
        ...
        "and which each skill references (body).\n"
    )
    out.append(f"- Generated on: **{_today()}**")
    ...
    out.append(
        ...
        "The unscoped agents above take every privileged action by default, and "
...
def main():
    ...
    if args.check:
        ...
        # The "Generated on" stamp changes every day and says nothing about
        # staleness, so compare the document with that one line normalized out.
        strip_stamp = lambda t: "\n".join(
