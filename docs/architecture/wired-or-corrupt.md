...
**PHASE 1 - complete corpus:** backfill every CLAUDE.md anchor (all ~50 rules) including PRESENCE rows for model-behavior. D2 now truly proves zero un-backed prose. This is the moment 100% COVERAGE first holds.
...
**PHASE 3 - the rename:** generate the codemod from the manifest `old_name` column, run `multi-object-rename-convergence`, add the name-reparse assert (parsed==declared), drive D4 to 0, delete `old_name`. Suffix soup gone, names lossless.

**PHASE 4 - liveness + honest ceiling:** standardize `trace-hook` → `.brain/trace.jsonl`; D6 STALE detection (WARN). Add EFFECTIVE probes for the ~3 cheap deterministic gates (cadence, secrets, qa-merge) only. Publish the Ledger to brain-digest. Stop here; do NOT over-wire model-behavior into fake gates.
