...
Bootstrap note: the first merge of that PR cannot carry a QA receipt, because the SubagentStop hook that writes receipts is inside it. That merge uses the operator's explicit `OCTO_QA_OK=1` bypass, once, alongside `OCTO_MERGE_APPROVE=<that PR>`, and the PR body carries the QA verdict as text. Every merge after it needs the receipt. `OCTO_QA_OK` waives the receipt only: it is scoped to the PR named in `OCTO_MERGE_APPROVE`, and a merge of any other number is denied (fixtures `violation_qa_ok_wrong_pr.json` / `benign_qa_ok_right_pr.json`).
